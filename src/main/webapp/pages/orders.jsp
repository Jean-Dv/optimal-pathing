...
<% int currentPage =(Integer) request.getSession().getAttribute("currentPage");
  ...
  <ol  class="flex justify-center gap-1 text-xs font-medium ml-56">
    <li>
      <a
        href="?page=<%= currentPage > 1 ? currentPage - 1 : 1 %>&status=<%= request.getParameter("status") %>"
    ...
    <% for(int i= 1;i<= totalPages;i++) {   %>
      <li>
        <a
          href="?page=<%= i %>&status=<%= request.getParameter("status") %>"
          class="block size-8 rounded border border-blue-100 bg-white text-center leading-8 text-gray-900"
        >
          <%= i %>
        </a>
      </li>
    ...
    <li>
      <a
        href="?page=<%= currentPage < totalPages ? currentPage + 1 : totalPages %>&status=<%= request.getParameter("status") %>"
        class="inline-flex size-8 items-center justify-center rounded border border-gray-100 bg-white text-gray-900 rtl:rotate-180"
      >
